    ...
    def update(self, output_results, img_info=None, img_size=None):
        ...
        if len(dets) > 0:
            ...
            detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, idx) for
                          (tlbr, s, idx) in zip(dets, scores_keep, det_idxes_keep)]
        else:
            detections = []
        ...
        if len(dets_second) > 0:
            ...
            detections_second = [STrack(STrack.tlbr_to_tlwh(tlbr), s, idx) for
                                 (tlbr, s, idx) in zip(dets_second, scores_second, det_idxes_second)]
        else:
            detections_second = []
        ...
        # print(f"Second match: {len(matches)}, {len(u_track)}, {len(u_detection_second)}.")
        ...
        # print(f"Unfonfirmed match: {len(matches)}, {len(u_unconfirmed)}, {len(u_detection)}.")
